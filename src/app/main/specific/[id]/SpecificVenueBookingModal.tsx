...
export default function SpecificVenueBookingModal({
  ...
  const [selectedDate, setSelectedDate] = useState<Date | null>(
    availableDates[2] ?? availableDates[0] ?? null,
  );
  const [selectedTime, setSelectedTime] = useState<string | null>(
    availableTimes[2] ?? availableTimes[0] ?? null,
  );
            ...
            >
              <div
                className={cn(
                  step === paymentStep
                    ? "h-full"
                    : "grid min-h-0 flex-1 gap-6 lg:grid-cols-[minmax(0,1fr)_240px]",
                )}
              >
                <div
                  className={cn(
                  ...
                  {step === serviceStep ? (
                    <div className="rounded-[28px] border border-slate-100 bg-white p-5 shadow-sm">
                      ...
                      <p className="mt-1 text-sm text-slate-500">
                        Choose the treatment you want to book today.
                      </p>
                ...
                {step !== paymentStep &&
                !(step === staffStep && !isServiceFirstFlow) ? (
                  <div className="rounded-[28px] border border-slate-100 bg-white p-4 shadow-sm h-fit">
                    ...
                    {(isServiceFirstFlow ? step >= 2 : step >= 1) &&
                    selectedStaff ? (
