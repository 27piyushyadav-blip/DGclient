...
import { CalendarDays, Check, ChevronLeft, ChevronRight, CreditCard, MapPin, ShieldCheck, Sparkles, Star, UserRound, Wallet } from "lucide-react";
...
//   }, [open, services, staff, availableDates, availableTimes, initialStep, preselectedService]);
...
useEffect(() => {
  ...
  const canGoNext = Boolean(
    (step === serviceStep && selectedService) ||
      ...
      (step === paymentStep && selectedDate && selectedTime && selectedPaymentMethod),
  );
...
const handleDateTimeConfirm = () => {
        ...
        <div className="grid max-h-[90vh] min-h-[680px] md:grid-cols-[280px_minmax(0,1fr)]">
          <div className="relative hidden overflow-hidden bg-gradient-to-br from-amber-950 via-amber-800 to-stone-950 p-6 text-white md:flex md:flex-col md:justify-end">
            ...
            <div className="relative space-y-6">
              <div>
                <p className="text-3xl font-semibold leading-tight">Escape the stress.</p>
                <p className="text-3xl font-semibold leading-tight text-white/90">Embrace the calm.</p>
              </div>
  ...
  <div className="border-b border-slate-100 px-6 pb-5 pt-6">
    <DialogTitle className="text-3xl font-bold text-slate-900">Book Your Appointment</DialogTitle>
    ...
    <div className="mt-6 flex items-center gap-3 overflow-x-auto">
      {stepConfig.map((item, index) => (
        <div key={item.id} className="flex min-w-fit items-center gap-3">
          ...
          <span className={cn("text-sm font-medium", step >= item.id ? "text-blue-600" : "text-slate-500")}>
            {item.label}
          </span>
          {index < stepConfig.length - 1 ? <div className="h-px w-16 bg-slate-200 md:w-28" /> : null}
        </div>
      ))}
    </div>
  </div>
    ...
    <div className={cn(
      ...
      <div className={cn(
        ...
        step === paymentStep && "h-full"
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              ...
                              isSelected ? "border-blue-600 bg-blue-50/70 shadow-sm" : "border-slate-200 hover:border-blue-300",
                            )}
                          >
                              ...
                              <div className="flex flex-1 flex-col">
                                <div className="flex items-start justify-between gap-3">
                                  <div>
                                    ...
                                    <p className="mt-1 text-xs text-slate-500">{duration} min</p>
                                  </div>
                                  <div
                                    className={cn(
                                ...
                                <p className="mt-3 text-lg font-bold text-blue-600">{service.price}</p>
                                
                              </div>
                            </div>
                            <p className="mt-auto pt-2 text-xs leading-5 text-slate-500">
                                  Therapeutic care tailored for relaxation, recovery and full-body comfort.
                                </p>
                          </button>
                        );
                      })}
                ...
                {step === staffStep ? (
                  <div className="rounded-[28px] border border-slate-100 bg-white p-5 shadow-sm">
                    <h3 className="text-xl font-semibold text-slate-900">Select Your Preferred Therapist</h3>
                      ...
                      <div className="overflow-hidden">
                        ...
                        >
                          {staff.map((member, index) => {
                            const isSelected = selectedStaff?.name === member.name;
                                    ...
                                    <>
                                      <div className="relative">
                                        ...
                                        <div
                                          className={cn(
                                            ...
                                            isSelected ? "border-blue-600 text-blue-600" : "border-slate-300 text-transparent",
                                          )}
                                        >
                                          <Check className="h-3.5 w-3.5" />
                                        </div>
                                      ...
                                      <div className="pt-4 text-center">
                                        ...
                                        <div className="mt-2 flex items-center justify-center gap-1 text-amber-400">
                                          ...
                                          <span className="ml-1 text-xs text-slate-500">({180 + index * 23})</span>
                                        </div>
                                        <p className="mt-3 text-xs leading-5 text-slate-500">Specializes in {specialties || member.role}</p>
                                      </div>
                                    </>
                                  ) : (
                                    <div className="relative flex flex-col items-center justify-center gap-4 text-center">
                                      ...
                                      <div className="min-w-0">
                                        ...
                                        <p className="mt-3 text-xs leading-5 text-slate-500">
                                          Specializes in {specialties || member.role}
                                        </p>
                                      </div>
                                    </div>
                                  )}
                        ...
                        <>
                          <button
                            ...
                            className={cn(
                              ...
                              scrollIndex === 0 ? "opacity-50 cursor-not-allowed" : "hover:bg-slate-50 hover:shadow-xl"
                            )}
                          >
                          ...
                          <button
                            ...
                            className={cn(
                              ...
                              scrollIndex >= maxScrollIndex ? "opacity-50 cursor-not-allowed" : "hover:bg-slate-50 hover:shadow-xl"
                            )}
                          >
                          ...
                          {Array.from({ length: totalDots }).map((_, idx) => (
                            <button
                              ...
                              className={cn(
                                ...
                                scrollIndex === idx ? "w-6 bg-blue-600" : "w-2 bg-slate-300 hover:bg-slate-400"
                              )}
                            />
                          ))}
                        </div>
                ...
                {step === paymentStep   ? (
                  <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_500px]">
                    <div className="rounded-[28px] border border-slate-100 bg-white p-5 shadow-sm max-h-[30rem] overflow-y-scroll scrollbar-thin scrollbar-thumb-slate-100">
                      ...
                      <div className="mt-5  gap-4 rounded-[22px] border border-slate-100 p-4 ">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-sm text-slate-500">60 min - {selectedService?.price}</p>
                      ...
                      <div className="mt-6">
                        <div className="flex items-center justify-between">
                          <h4 className="font-semibold text-slate-900">Selected Therapist</h4>
                          <button
                            type="button"
                      ...
                      <div className="mt-6">
                        <h4 className="font-semibold text-slate-900">Select Date & Time</h4>
                    ...
                    <div className="rounded-[28px] border border-slate-100 bg-white p-5 shadow-sm max-h-[30rem]">
                      <h3 className="text-xl font-semibold text-slate-900">Your Booking Summary</h3>
  ...
  <div className="flex-1">
    <div className="space-y-3 text-sm">
      {bookingSummary.map((item) => (
        <div key={item.label} className="flex items-start justify-between gap-4">
          ...
          <span className="text-right font-semibold text-slate-900 text-xs">{item.value}</span>
        </div>
      ))}
    </div>
    ...
    <div className="mt-3 rounded-xl border border-slate-100 p-2">
      ...
      <div className="mt-2 flex items-start gap-2">
        ...
        <div>
          <p className="text-xs font-semibold text-slate-900">{venue.name}</p>
  ...
  <div className="flex-1">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-slate-800">{method.label}</span>
              </div>
            </button>
          );
        })}
    ...
    <div className="mt-3 space-y-1.5 rounded-xl bg-slate-50 p-3">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-semibold text-slate-900">${bookingPrice}</span>
      </div>
      <div className="flex items-center justify-between">
        <span className="text-xs text-slate-500">Taxes & Fees</span>
        <span className="text-xs font-semibold text-slate-900">${taxes}</span>
      </div>
...
<div className="mt-6">
  ...
  <div className="mt-3 grid grid-cols-3 gap-3 sm:grid-cols-4">
    {timeSlots.map((time) => {
      ...
      const isDisabled = !tempSelectedDate || time === "12:30 PM" || time === "1:00 PM";
